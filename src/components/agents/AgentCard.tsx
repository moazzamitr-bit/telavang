...
const statusMap = {
  ...
  disabled: { label: 'خاموش', tone: 'default' as const },
}
    ...
    <button type="button" onClick={onSelect} className="w-full text-right">
      <Card
        className={cn(
          ...
          selected && 'ring-2 ring-telavang/40',
          ...
          <div className="mt-4 space-y-2.5">
            ...
            <div className="flex items-center justify-between text-xs">
              <span className="text-ink-muted">
                آخرین فعالیت: {formatRelativeTime(agent.lastActivityMinutes)}
              </span>
            ...
            <div className="flex items-center justify-between text-[11px] text-ink-muted">
              <span>{toPersianDigits(agent.skillIds.length)} مهارت</span>
              <span>خودمختاری {toPersianDigits(agent.autonomy)}٪</span>
            </div>
